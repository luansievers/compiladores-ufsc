...
public class TypeCheck extends VarCheck {
	...
	public void TypeCheckSwitchNode(SwitchNode x) {
        ...
        try {
            nesting++; // incrementa o aninhamento
            TypeCheckSwitchCasesNode(x.switchcases);
        } catch (SemanticException e) {
            System.out.println(e.getMessage());
            foundSemanticError++;
        }
	...
	// ------------------------- lista de cases -----------------------
	public void TypeCheckSwitchCasesNode(ListNode x) throws SemanticException {
		...
		try {
            ...
            TypeCheckSwitchCaseNode((SwitchCaseNode) x.node);
        } catch (SemanticException e) {
            System.out.println(e.getMessage());
            foundSemanticError++;
		...
		TypeCheckSwitchCasesNode(x.next);

	}
	
	// ------------------------- comando switchcase -----------------------
	public void TypeCheckSwitchCaseNode(SwitchCaseNode x) throws SemanticException {
		type t;
    ...
    public void TypeCheckBreakNode(BreakNode x) throws SemanticException {
        ...
        if (nesting <= 0) {
            throw new SemanticException(x.position,
                "break not in a for or switch statement");
        }
    }
    ...
    public type TypeCheckRelationalNode(RelationalNode x)
        ...
        if ((t1.ty == INT_TYPE) && (t2.ty == INT_TYPE)) {
            ...
            return new type(BOOLEAN_TYPE, 0);
        } else if ((t1.ty == DOUBLE_TYPE) && (t2.ty == DOUBLE_TYPE)) {
            return new type(BOOLEAN_TYPE, 0);
        }
        ...
        if ((op != CompiladorConstants.EQ) && (op != CompiladorConstants.NEQ) &&
                (t1.dim > 0)) {
            throw new SemanticException(x.position,
        ...
        if ((isSubClass(t2.ty, t1.ty) || isSubClass(t1.ty, t2.ty)) &&
                ((op == CompiladorConstants.NEQ) || (op == CompiladorConstants.EQ))) {
            return new type(BOOLEAN_TYPE, 0);
        }
    ...
    public type TypeCheckAddNode(AddNode x) throws SemanticException {
        ...
        if ((op == CompiladorConstants.PLUS) && ((i + s) == 2)) {
    ...
    public type TypeCheckUnaryNode(UnaryNode x) throws SemanticException {
        ...
        if (t.ty == INT_TYPE) {
            return new type(INT_TYPE, 0);
        } else if (t.ty == FLOAT_TYPE) {
            return new type(FLOAT_TYPE, 0);
        } else if (t.ty == DOUBLE_TYPE) {
            return new type(DOUBLE_TYPE, 0);
        } else {
			throw new SemanticException(x.position,
	...
	// -------------------------- Literal inteiro ----------------------
    public type TypeCheckIntLitNode(IntLitNode x) throws SemanticException {
        int k;
        ...
        try {
            ...
            throw new SemanticException(x.position, "Invalid int literal");
        }
	...
	// ------------------------ Literal string ----------------------------
    public type TypeCheckStringLitNode(StringLitNode x) {
        if (x == null) {
            return null;
        }
	...
	// ------------------------ Literal float ----------------------------
    public type TypeCheckFloatLitNode(FloatLitNode x) throws SemanticException{
		float k;
		...
		// tenta transformar imagem em número float
        try {
            k = Float.parseFloat(x.position.image);
        } catch (NumberFormatException e) { // se deu erro, formato e inválido (possivelmente fora dos limites)
            throw new SemanticException(x.position, "Invalid float literal");
        }
	...
	// ------------------------ Literal char ----------------------------
    public type TypeCheckCharLitNode(CharLitNode x) {
        if (x == null) {
            return null;
        }
	...
	// ------------------------ Literal double ----------------------------
    public type TypeCheckDoubleLitNode(DoubleLitNode x) throws SemanticException {
		double k;
		...
		// tenta transformar imagem em número double
        try {
            k = Double.parseDouble(x.position.image);
        } catch (NumberFormatException e) { // se deu erro, formato e inválido (possivelmente fora dos limites)
            throw new SemanticException(x.position, "Invalid double literal");
        }
	...
	// ------------------------ Literal boolean ----------------------------
    public type TypeCheckBooleanLitNode(BooleanLitNode x) {
        if (x == null) {
            return null;
        }
    ...
    public type TypeCheckExpreNode(ExpreNode x) throws SemanticException {
        if (x instanceof NewObjectNode) {
            ...
            return TypeCheckIntLitNode((IntLitNode) x);
        } else if (x instanceof StringConstNode) {
            return TypeCheckStringConstNode((StringConstNode) x);
        } else if (x instanceof StringLitNode) {
            return TypeCheckStringLitNode((StringLitNode) x);
        } else if (x instanceof FloatLitNode) {
            return TypeCheckFloatLitNode((FloatLitNode) x);
        } else if (x instanceof CharLitNode) {
            return TypeCheckCharLitNode((CharLitNode) x);
        } else if (x instanceof DoubleLitNode) {
            return TypeCheckDoubleLitNode((DoubleLitNode) x);
        } else if (x instanceof BooleanLitNode) {
            return TypeCheckBooleanLitNode((BooleanLitNode) x);
